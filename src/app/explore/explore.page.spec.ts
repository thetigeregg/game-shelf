...
describe('ExplorePage explore modes UX', () => {
  ...
  it('covers popularity helper branches for title, cover, platform labels, release year, and badges', () => {
    const page = createPage() as unknown as {
      localGameCacheByIdentity: Map<string, unknown>;
      buildIdentityKey: (igdbGameId: string, platformIgdbId: number) => string;
      getPopularityTitle: (item: { id: string; platformIgdbId: number; name: string }) => string;
      getPopularityCoverUrl: (item: {
        id: string;
        platformIgdbId: number;
        coverUrl: string | null;
      }) => string;
      getPopularityPlatformLabel: (item: {
        platformIgdbId: number;
        platforms?: Array<{ id: number; name: string }>;
      }) => string;
      getPopularityReleaseYear: (item: { firstReleaseDate: number | null }) => number | null;
      getPopularityBadges: (item: {
        popularityScore: number;
        rating?: number;
      }) => Array<{ text: string; color: string }>;
    };

    page.localGameCacheByIdentity.set(page.buildIdentityKey('10', 6), {
      customTitle: '  Local Custom  ',
      title: 'Base Title',
      customCoverUrl: 'https://example.com/custom.jpg',
      coverUrl: 'https://example.com/base.jpg',
    });
    page.localGameCacheByIdentity.set(page.buildIdentityKey('11', 6), {
      customTitle: '   ',
      title: 'Library Title',
      customCoverUrl: null,
      coverUrl: 'https://example.com/library.jpg',
    });

    expect(page.getPopularityTitle({ id: '10', platformIgdbId: 6, name: 'Feed Title' })).toBe(
      'Local Custom'
    );
    expect(page.getPopularityTitle({ id: '11', platformIgdbId: 6, name: 'Feed Title' })).toBe(
      'Library Title'
    );
    expect(page.getPopularityTitle({ id: '12', platformIgdbId: 6, name: '  Feed Title  ' })).toBe(
      'Feed Title'
    );
    expect(page.getPopularityTitle({ id: '13', platformIgdbId: 6, name: '   ' })).toBe('Game #13');

    expect(
      page.getPopularityCoverUrl({
        id: '10',
        platformIgdbId: 6,
        coverUrl: 'https://example.com/feed.jpg',
      })
    ...
    ).toBe('https://example.com/library.jpg');
    expect(
      page.getPopularityCoverUrl({
        id: '12',
        platformIgdbId: 6,
    ...
    ).toBe('https://example.com/feed.jpg');
    expect(page.getPopularityCoverUrl({ id: '13', platformIgdbId: 6, coverUrl: null })).toBe(
      'assets/icon/placeholder.png'
    );

    expect(
      page.getPopularityPlatformLabel({
        platformIgdbId: 6,
        platforms: [
          { id: 6, name: 'PC' },
          { id: 48, name: 'PS4' },
    ...
    expect(page.getPopularityPlatformLabel({ platformIgdbId: 99 })).toBe('Platform 99');

    expect(page.getPopularityReleaseYear({ firstReleaseDate: 0 })).toBeNull();
    expect(page.getPopularityReleaseYear({ firstReleaseDate: 1_578_729_600 })).toBe(2020);
    expect(page.getPopularityReleaseYear({ firstReleaseDate: Number.MAX_SAFE_INTEGER })).toBeNull();

    expect(page.getPopularityBadges({ popularityScore: 88.84, rating: 91.25 })).toEqual([
      { text: 'Popularity 88.8', color: 'primary' },
      { text: 'IGDB 91.3', color: 'success' },
    ]);
    expect(
      page.getPopularityBadges({ popularityScore: 42.04, rating: Number.POSITIVE_INFINITY })
    ).toEqual([{ text: 'Popularity 42.0', color: 'primary' }]);
  });
  ...
  it('covers merged platform labels and catalog/detail identity helper branches', () => {
    const page = createPage() as unknown as {
      activeLanesResponse: {
        lane: 'overall';
      ...
      selectedLaneKey: 'overall';
      localGameCacheByIdentity: Map<string, unknown>;
      recommendationDisplayMetadata: Map<string, unknown>;
      catalogCache: Map<string, unknown>;
      selectedGameDetail: { igdbGameId?: string; platformIgdbId?: number } | null;
      buildIdentityKey: (igdbGameId: string, platformIgdbId: number) => string;
      getMergedPlatformLabels: (item: {
        igdbGameId: string;
        platformIgdbId: number;
      }) => string | null;
      getCatalogResult: (igdbGameId: string) => unknown;
      getLocalGameByIdentity: (igdbGameId: string, platformIgdbId: number) => unknown;
      hasSelectedDetailIdentity: (expectedIdentityKey: string) => boolean;
    };

    page.activeLanesResponse = {
      lane: 'overall',
      items: [{ igdbGameId: '900', platformIgdbId: 6 }],
    };
    page.selectedLaneKey = 'overall';

    expect(page.getMergedPlatformLabels({ igdbGameId: '900', platformIgdbId: 6 })).toBeNull();

    page.activeLanesResponse = {
      lane: 'overall',
      items: [
        { igdbGameId: '900', platformIgdbId: 6 },
        { igdbGameId: '900', platformIgdbId: 48 },
        { igdbGameId: '900', platformIgdbId: 167 },
      ],
    };
    page.localGameCacheByIdentity.set(page.buildIdentityKey('900', 6), {
      platform: 'PC',
      platformIgdbId: 6,
    });
    page.recommendationDisplayMetadata.set(page.buildIdentityKey('900', 48), {
      platformLabel: 'PS4',
    });

    expect(page.getMergedPlatformLabels({ igdbGameId: '900', platformIgdbId: 6 })).toBe(
      'PC / PS4 / Platform 167'
    );

    const cachedCatalog = { igdbGameId: '900', title: 'Cached' };
    page.catalogCache.set('900', cachedCatalog);
    expect(page.getCatalogResult(' 900 ')).toBe(cachedCatalog);
    expect(page.getCatalogResult('   ')).toBeNull();

    expect(page.getLocalGameByIdentity('900', 6)).toEqual({ platform: 'PC', platformIgdbId: 6 });
    expect(page.getLocalGameByIdentity('900', 99)).toBeNull();

    page.selectedGameDetail = null;
    expect(page.hasSelectedDetailIdentity(page.buildIdentityKey('900', 6))).toBe(false);
    page.selectedGameDetail = { igdbGameId: '', platformIgdbId: 6 };
    expect(page.hasSelectedDetailIdentity(page.buildIdentityKey('900', 6))).toBe(false);
    page.selectedGameDetail = { igdbGameId: '900', platformIgdbId: 4.5 };
    expect(page.hasSelectedDetailIdentity(page.buildIdentityKey('900', 6))).toBe(false);
    page.selectedGameDetail = { igdbGameId: '900', platformIgdbId: 6 };
    expect(page.hasSelectedDetailIdentity(page.buildIdentityKey('900', 6))).toBe(true);
  });

  it('covers ignore confirmation cancellation and title fallback branches', async () => {
    const page = createPage() as unknown as {
      activeDetailRecommendation: { igdbGameId: string; platformIgdbId: number } | null;
      selectedGameDetail: { igdbGameId: string; title: string; platformIgdbId: number } | null;
      confirmIgnoreSelectedGameRecommendation: () => Promise<void>;
    ...
    page.activeDetailRecommendation = { igdbGameId: '700', platformIgdbId: 6 };
    page.selectedGameDetail = { igdbGameId: '700', title: '<Alpha>', platformIgdbId: 6 };
    alertControllerMock.create.mockResolvedValueOnce({
      present: vi.fn().mockResolvedValue(undefined),
      onDidDismiss: vi.fn().mockResolvedValue({ role: 'cancel' }),
    });
    ...
    expect(alertControllerMock.create).toHaveBeenCalledWith(
      expect.objectContaining({
        message: 'Hide "&lt;Alpha&gt;" from recommendation lists?',
      })
    );
    expect(recommendationIgnoreServiceMock.ignoreGame).not.toHaveBeenCalled();

    page.activeDetailRecommendation = null;
    page.selectedGameDetail = { igdbGameId: '701', title: '   ', platformIgdbId: 6 };
    alertControllerMock.create.mockResolvedValueOnce({
      present: vi.fn().mockResolvedValue(undefined),
