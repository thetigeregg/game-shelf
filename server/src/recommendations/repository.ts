...
export class RecommendationRepository {
        ...
        DO UPDATE
          SET payload = EXCLUDED.payload || jsonb_strip_nulls(
                jsonb_build_object(
                  ...
                  'psPricesPriceRegularAmount', games.payload->'psPricesPriceRegularAmount',
                  'psPricesPriceDiscountPercent', games.payload->'psPricesPriceDiscountPercent',
                  'psPricesPriceIsFree', games.payload->'psPricesPriceIsFree',
  ...
  /* prettier-ignore */ async failStaleRunningRuns(params?: { maxAgeMinutes?: number; errorMessage?: string; target?: RecommendationTarget | null }): Promise<{ failedCount: number; runIds: number[] }> {
    const maxAgeMinutes = Number.isInteger(params?.maxAgeMinutes)
      ? Math.max(1, params?.maxAgeMinutes ?? 0)
      : 30;
    let errorMessage = 'orphaned RUNNING run recovered after worker loss';
    if (typeof params?.errorMessage === 'string' && params.errorMessage.trim().length > 0) {
      errorMessage = params.errorMessage.trim();
    }
    const target = params?.target ?? null;
    const result = await this.pool.query<{ id: number }>(`UPDATE recommendation_runs SET status = 'FAILED', finished_at = NOW(), error = COALESCE(error, $3) WHERE status = 'RUNNING' AND started_at < (NOW() - make_interval(mins => $1)) AND ($2::text IS NULL OR target = $2) AND NOT EXISTS (SELECT 1 FROM background_jobs WHERE background_jobs.job_type = 'recommendations_rebuild' AND background_jobs.status = 'running' AND COALESCE(background_jobs.payload->>'target', '') = recommendation_runs.target) RETURNING id`, [maxAgeMinutes, target, errorMessage]);
