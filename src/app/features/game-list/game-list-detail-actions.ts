...
export function hasHltbData(game: GameEntry): boolean {
  return isPositiveNumber(game.hltbMainHours)
    || isPositiveNumber(game.hltbMainExtraHours)
    || isPositiveNumber(game.hltbCompletionistHours);
}

export function normalizeGameStatus(value: string | GameStatus | null | undefined): GameStatus | null {
...
export function normalizeGameRating(value: number | string | GameRating | null | undefined): GameRating | null {
...
export function buildTagInput(tag: Tag, selectedTagIds: number[]): { type: 'checkbox'; label: string; value: string; checked: boolean } {
