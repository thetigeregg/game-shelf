...
|     150 | Turbografx-16/PC Engine CD          | `pce`        | `__pid-150`                             | `pce/pce-bios.zip` (CD BIOS members at archive root; zip convention for multi-file BIOS sets)                    |
...
- Default URLs and SRI pins live in `src/app/core/config/emulatorjs.constants.ts`. After each **`game-shelf-assets`** EmulatorJS release deploy, update pins from the published manifest (`EMULATORJS_ASSETS_MANIFEST_URL` in that file: use `version`, `basePath`, and `entrypoints.loader.sri.sha384` for the deployed `loader.js`). Production builds may inject overrides through `scripts/write-environment-prod.sh` (`EMULATORJS_PATH_TO_DATA_PROD`, `EMULATORJS_LOADER_INTEGRITY_PROD`); values must satisfy the allowlist enforced in `src/app/core/utils/emulatorjs-play-url.ts` and the play shell.
...
```bash
npx devx worktree db seed-apply --force
```
