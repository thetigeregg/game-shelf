...
- `BACKGROUND_WORKER_MODE` (optional; defaults `general`; used by `worker-general`)
- `BACKGROUND_WORKER_MODE_RECOMMENDATIONS` (optional; defaults `recommendations`; used by `worker-recommendations`)
...
- `BACKGROUND_JOBS_LOCK_HEARTBEAT_SECONDS` (optional; default `30`; consumed by `worker-general`)
