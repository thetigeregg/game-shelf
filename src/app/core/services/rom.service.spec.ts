...
import { RomService, ROM_OVERRIDES_STORAGE_KEY } from './rom.service';
...
describe('RomService', () => {
  ...
  it('builds rom url from client base on native even when server returns loopback url', async () => {
    ...
    const req = httpMock.expectOne(
      (request) => request.url === `${environment.gameApiBaseUrl}/v1/roms/resolve`
    );
    req.flush({
      status: 'matched',
      bestMatch: {
        ...
        score: 0.95,
        url: 'http://127.0.0.1:10028/roms/PlayStation%202__pid-8/God%20of%20War%20II.iso',
      },
      candidates: [],
    });
    ...
    await expect(promise).resolves.toEqual({
      ...
      bestMatch: {
        ...
        relativePath: 'PlayStation 2__pid-8/God of War II.iso',
        score: 0.95,
        url: `${environment.romsBaseUrl}/PlayStation%202__pid-8/God%20of%20War%20II.iso`,
      },
      candidates: [],
      unavailable: false,
      reason: null,
  ...
  it('calls resolve endpoint and preserves override preference path', async () => {
    ...
    const req = httpMock.expectOne((request) => {
      return (
        request.url === `${environment.gameApiBaseUrl}/v1/roms/resolve` &&
    ...
    expect(
      outboxWriter.calls.some((call) => {
        return (
          ...
          (call.payload as { key?: string }).key === ROM_OVERRIDES_STORAGE_KEY
        );
      })
    ).toBe(true);
  });
  ...
  it('clears override and queues setting delete when map is empty', () => {
    ...
    expect(localStorage.getItem(ROM_OVERRIDES_STORAGE_KEY)).toBeNull();
    expect(
      outboxWriter.calls.some((call) => {
        return (
          call.entityType === 'setting' &&
  ...
  it('encodes semicolons in resolve query params', async () => {
    ...
    const req = httpMock.expectOne((request) => {
      return (
        ...
        `${environment.gameApiBaseUrl}/v1/roms/resolve?igdbGameId=11393&platformIgdbId=46&title=Chaos%3BChild`
      );
    });
    req.flush({
      status: 'none',
  ...
  it('returns empty search list when platform id is invalid without HTTP call', async () => {
    await expect(firstValueFrom(service.searchRoms(0, 'mario'))).resolves.toEqual({
      items: [],
      unavailable: false,
      reason: null,
    });

    httpMock.expectNone(`${environment.gameApiBaseUrl}/v1/roms/search`);
  });

  it('maps search response and fallback URL when candidate URL is missing', async () => {
    const promise = firstValueFrom(service.searchRoms(8, 'god'));

    const req = httpMock.expectOne((request) => {
      return (
        request.url === `${environment.gameApiBaseUrl}/v1/roms/search` &&
        request.params.get('platformIgdbId') === '8' &&
