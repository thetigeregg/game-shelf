...
function makeGame(
  partial: Partial<GameEntry> & Pick<GameEntry, 'igdbGameId' | 'platformIgdbId' | 'title'>
): GameEntry {
  return {
...
describe('GameListFilteringEngine UI behavior', () => {
  ...
  it('treats Family Computer Disk System as NES and e-Reader / Card-e Reader as Game Boy Advance', () => {
    ...
    expect(engine.extractPlatforms(games).sort()).toEqual(
      ['Nintendo Entertainment System', 'Game Boy Advance'].sort()
    );
  ...
  it('excludes games matching the excluded compatibility filter', () => {
    const games: GameEntry[] = [
      makeGame({
        igdbGameId: '1',
        platformIgdbId: 21,
    ...
    const result = engine.applyFiltersAndSort(
      ...
      {
        ...DEFAULT_GAME_LIST_FILTERS,
        excludedCompatibility: ['incomplete'],
      },
      ''
    );
    ...
    expect(result.map((game) => game.title).sort()).toEqual([
      'Perfect Game',
      'Untracked Platform Game',
    ]);

    const guaranteedExcluded = engine.applyFiltersAndSort(
      games,
      {
        ...DEFAULT_GAME_LIST_FILTERS,
        excludedCompatibility: ['guaranteed'],
      },
      ''
    );

    expect(guaranteedExcluded.map((game) => game.title).sort()).toEqual([
      'Incomplete Game',
      'Perfect Game',
    ]);
  });
